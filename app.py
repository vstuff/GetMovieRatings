...
# CSS for professional, mobile-friendly styling—consistent and polished look
st.markdown("""
    <style>
    body { 
...
# App title and description—sets the tone at the top
...
with st.form(key="movie_form"):
    ...
    # Center the submit button for a polished look, returns True on submit
    submit_button = st.form_submit_button(label="Get Ratings & More", help="Click to search!")

# Process submission when form is submitted and title is provided
if submit_button and movie_title:
    # Always display search links—gives users options regardless of result
    ...
    if data["Response"] != "True":
        ...
        if language and language.lower() != searched_language.lower():
            ...
            if not lang_match:
                # Warn if language input doesn’t match—proceed anyway
        ...
        # Two-column layout: poster on left, ratings on right
    ...
    # Always display search links before submission—default to placeholder if no title
    google_search_url = f"https://www.google.com/search?q={movie_title.replace(' ', '+')}+movie+ratings" if movie_title else "#"
    imdb_search_url = f"https://www.imdb.com/find?q={movie_title.replace(' ', '+')}&s=tt" if movie_title else "#"
    letterboxd_search_url = f"https://letterboxd.com/search/{movie_title.replace(' ', '+')}/" if movie_title else "#"
    st.markdown(
        f"<div class='search-links'>"
        f"<a href='{google_search_url}' target='_blank'>Google Search</a>"
        f"<a href='{imdb_search_url}' target='_blank'>IMDb Search</a>"
